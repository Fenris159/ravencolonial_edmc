...
#       Lowercase paths; match SrvSurvey / typical host routing.
#   contribute_cargo       POST   /api/project/{buildId}/contribute/{cmdr}
#       Body: Cargo map; commander delivery history ONLY; does not change project remaining need.
#   patch_project_update   PATCH  /api/project/{buildId}
#       Body: ProjectUpdate (+ colonisationConstructionDepot); authoritative remaining need from journal.
#   (POST /api/project/{buildId}/supply/{cmdr} subtracts remaining need then contributes; journal-aware
#   clients use PATCH depot + /contribute instead. This plugin never calls /supply.)
...
#   get_system_architect   GET    /api/v2/system/{nameOrNum}/architect
#       Response: string, or wrapped dict handled in code.
...
#   update_fc_cargo        POST   /api/fc/{marketId}/cargo
#       + header rcc-key only (SrvSurvey `updateCargoFC`; key scopes commander)
...
class RavencolonialAPIClient:
    ...
    def get_project(self, system_address: int, market_id: int) -> Optional[Dict]:
        """Get project details for a system/station via lowercase SrvSurvey-style route."""
        try:
    ...
    def mark_project_complete(self, build_id: str) -> bool:
        ...
        try:
            ...
            logger.debug("Request timeout: 10s")
